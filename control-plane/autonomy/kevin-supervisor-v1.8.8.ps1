...
# Kevin Supervisor v1.8.8 Governed Autonomy Continuation Controller - Native OpenClaw Runtime
...
function Get-PublicContinuation([object]$State) {
    $allowed=@('IDLE_NO_ELIGIBLE_DEMAND','WAITING_ITEM_BUDGETS','CHECK_ONLY_ELIGIBLE','AGENT_TURN_COMPLETED_NOT_OUTCOME_PROOF','CONTROLLER_ERROR')
...
if ($SelfTest) {
    ...
    Write-Host 'KEVIN SUPERVISOR v1.8.8 SELFTEST PASS selector_first=true gateway_agent=fixed-main gateway_rpc_only=true gateway_probe_retries=3 main_preflight=true no_forge_dispatch=true anti_spin=true openclaw_native_node=true shell_shim_bypassed=true native_timeout=180s arbitrary_shell=false authority_expansion=false'
    Write-Host 'KEVIN SUPERVISOR v1.8.8 DURABLE-BUDGET PASS per_item_history=true attempts_before_effect=true corruption_fails_closed=true alternating_items_bounded=true'
    Write-Host 'KEVIN SUPERVISOR v1.8.8 WORK-CONSERVATION PASS fingerprint_scoped_cooldown=true alternative_reselection=true local_exclusion_only=true canonical_queue_unchanged=true no_global_idle_with_alternative=true authority_expansion=false'
    exit 0
}
