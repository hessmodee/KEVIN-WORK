...
# Kevin Supervisor v1.8.11 Capability-Aware Continuation Controller - Native OpenClaw Runtime (+ mission-lease wire)
...
$AdmissionNote = 'v1.8.11 routes exact PROVEN skill keys to invocation before Skill Lab; never calls fixed:main for owner-value invocation work'
...
function Get-PublicContinuation([object]$State) {
    ...
    $out=[ordered]@{schema=1;kind='kevin-autonomy-continuation-public';version='1.8.11';generated_at=[string]$State.at;status=[string]$State.status;safe_for_public_repo=$true;outcome_proven=$false;model_call_counts_as_accomplishment=$false;tool_calls=$null;history=@();history_error=$false}
...
if ($SelfTest) {
    ...
    Write-Host 'KEVIN SUPERVISOR v1.8.11 SELFTEST PASS identity_key_budget=true annotation_excluded=true material_boolean_excluded=true selector_first=true capability_router=true skill_lab_not_main=true owner_value_skills_not_main=true proven_skill_before_skill_lab=true invocation_not_main=true public_history_skip_invalid=true deferred_reasons_published=true route_before_turn_charge=true desktop_not_toolless_main=true gateway_agent=fixed-main gateway_rpc_only=true gateway_probe_retries=3 main_preflight=true no_forge_dispatch=true anti_spin=true openclaw_native_node=true shell_shim_bypassed=true native_timeout=180s arbitrary_shell=false authority_expansion=false production_install=false'
    Write-Host 'KEVIN SUPERVISOR v1.8.11 DURABLE-BUDGET PASS per_item_history=true attempts_before_effect=true corruption_fails_closed=true alternating_items_bounded=true'
    Write-Host 'KEVIN SUPERVISOR v1.8.11 WORK-CONSERVATION PASS fingerprint_scoped_cooldown=true alternative_reselection=true local_exclusion_only=true canonical_queue_unchanged=true no_global_idle_with_alternative=true authority_expansion=false'
    exit 0
}
