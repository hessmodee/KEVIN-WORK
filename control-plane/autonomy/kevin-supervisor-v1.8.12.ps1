...
# Kevin Supervisor v1.8.12 Capability-Aware Continuation Controller - Native OpenClaw Runtime (+ mission-lease wire)
# Worker native errors fail-closed as BLOCKED_INVOCATION_RUNTIME so the scheduler is not crashed.
...
$AdmissionNote = 'v1.8.12 routes exact PROVEN skill keys to invocation before Skill Lab; worker native errors fail-closed as BLOCKED_INVOCATION_RUNTIME; never calls fixed:main for owner-value invocation work'
...
$LeaseBridge = Join-Path $Workspace 'ControlPlane\Invoke-KevinMissionLease.ps1'
$LeaseStore = Join-Path $Runtime 'mission-leases-v1.json'
...
function Get-ItemFingerprint([hashtable]$Paths,[string]$Id) {
    ...
    foreach($name in @('id','program','lane','status','authority_class','failure_family','acceptance_criteria','dependencies_ready','blocked','failure_attempts')){
...
function Invoke-MissionLease([string]$LeaseAction, [string]$MissionId = '', [string]$EvidenceUri = '', [string]$Reason = 'COMPLETE') {
    # Additive P0.3 wire: fail-soft so lease issues never widen authority or halt GREEN selection.
    if (-not (Test-Path -LiteralPath $LeaseBridge -PathType Leaf)) { return $null }
    try {
        $args = @('-NoProfile', '-File', $LeaseBridge, '-Action', $LeaseAction, '-StorePath', $LeaseStore, '-Holder', $LeaseHolder)
        if ($MissionId) { $args += @('-MissionId', $MissionId) }
        if ($EvidenceUri) { $args += @('-EvidenceUri', $EvidenceUri) }
        if ($LeaseAction -eq 'Release') { $args += @('-Reason', $Reason) }
...
function Get-PublicContinuation([object]$State) {
    $allowed=@('IDLE_NO_ELIGIBLE_DEMAND','WAITING_ITEM_BUDGETS','CHECK_ONLY_ELIGIBLE','AGENT_TURN_COMPLETED_NOT_OUTCOME_PROOF','CONTROLLER_ERROR','ROUTED_TO_SKILL_LAB','ROUTED_TO_ENGINEERING_RELAY','ROUTED_TO_PROVEN_SKILL_INVOCATION','BLOCKED_INVOCATION_RUNTIME','BLOCKED_DESKTOP_CAPABILITY')
    if($allowed-notcontains[string]$State.status){throw 'nonterminal controller state is not publishable'}
    $out=[ordered]@{schema=1;kind='kevin-autonomy-continuation-public';version='1.8.12';generated_at=[string]$State.at;status=[string]$State.status;safe_for_public_repo=$true;outcome_proven=$false;model_call_counts_as_accomplishment=$false;tool_calls=$null;history=@();history_error=$false}
    ...
    $histStatuses=@('IN_PROGRESS','MIGRATED_LEGACY','AGENT_TURN_COMPLETED_NOT_OUTCOME_PROOF','ROUTED_TO_SKILL_LAB','ROUTED_TO_ENGINEERING_RELAY','ROUTED_TO_PROVEN_SKILL_INVOCATION','BLOCKED_INVOCATION_RUNTIME','BLOCKED_DESKTOP_CAPABILITY')
...
function Invoke-Cycle {
    ...
    try {
        ...
        if($null-eq$sel.selection){
            [void](Invoke-MissionLease 'Recover')
        ...
        if($null-eq$sel.selection){
            if($id){[void](Invoke-MissionLease 'Release' $id '' 'WAITING_ITEM_BUDGETS')}; Save-Latest 'WAITING_ITEM_BUDGETS' @{eligible_count=$initialEligible;deferred=$deferred;work_conservation='ALL_ELIGIBLE_ITEMS_HAVE_RECORDED_DEFERRAL'}|Out-Null
            return
        }
        ...
        try {
            ...
            if ($selectedRow.Count -eq 1) {
                ...
                if ($selectedRow[0].PSObject.Properties.Name -contains 'required_skill_key') { $requiredSkillKey = [string]$selectedRow[0].required_skill_key }
                if ($selectedRow[0].PSObject.Properties.Name -contains 'proven_skill_key' -and -not $requiredSkillKey) { $requiredSkillKey = [string]$selectedRow[0].proven_skill_key }
            }
        } catch {
            throw ('capability route precheck failed: ' + (Safe-Text $_.Exception.Message))
        }
        $selectionRecord = [ordered]@{
            schema=1;kind='kevin-autonomy-selection';selected_at=$now.ToString('o');id=$id;program=[string]$sel.selection.program;lane=[string]$sel.selection.lane;score=[double]$sel.selection.score;fingerprint=$finger;authority_effect='NONE_SELECTION_ONLY';source='deterministic kevin-work-selector-v1.1';deferred=$deferred;required_skill_key=$requiredSkillKey
        }
        $skillKeyOk = ($requiredSkillKey -match '^[A-Za-z0-9._-]{4,80}@[A-Za-z0-9._-]{1,32}$')
        if ($skillKeyOk) {
            Write-JsonAtomic $SelectionPath $selectionRecord
            $workerCandidates = @(
                (Join-Path $Workspace 'ControlPlane\kevin-proven-skill-invoke-worker-v1.ps1'),
                (Join-Path $Workspace 'control-plane\autonomy\kevin-proven-skill-invoke-worker-v1.ps1')
            )
            $workerPath = $null
            foreach ($cand in $workerCandidates) {
                if (Test-Path -LiteralPath $cand -PathType Leaf) { $workerPath = $cand; break }
            }
            if (-not $workerPath) {
                Save-Latest 'BLOCKED_INVOCATION_RUNTIME' @{ selected_id=$id; fingerprint=$finger; eligible_count=$initialEligible; deferred=$deferred; worker='proven-skill-invocation'; required_skill_key=$requiredSkillKey; program=$routeProgram; forbid_fixed_main=$true; turn_charged=$false; reason='PROVEN_SKILL_INVOCATION_WORKER_NOT_INSTALLED'; next_resolution='QUALIFY_AND_ENABLE_INVOCATION_RUNTIME'; truth_boundary='Exact PROVEN skill requirement is present. Invocation runtime is not independently effective on this machine. Do not send this work to Skill Lab replay or tool-less fixed:main.' } | Out-Null
                return
            }
            $workerArgs = @('-NoProfile', '-File', $workerPath, '-WorkId', $id, '-SkillKey', $requiredSkillKey, '-RequestId', ('invoke-' + $id))
            $oldWorkerPref = $ErrorActionPreference
            $workerOut = $null
            $workerCode = 1
            try {
                $ErrorActionPreference = 'Continue'
                $workerOut = & powershell @workerArgs 2>&1
                if ($null -ne $LASTEXITCODE) { $workerCode = [int]$LASTEXITCODE }
            } catch {
                $workerOut = @($workerOut) + @((Safe-Text $_.Exception.Message))
                if ($null -ne $LASTEXITCODE -and [int]$LASTEXITCODE -ne 0) { $workerCode = [int]$LASTEXITCODE } else { $workerCode = 1 }
            } finally {
                $ErrorActionPreference = $oldWorkerPref
            }
            $workerText = Safe-Text (($workerOut | Out-String).Trim()) 800
            if ($workerCode -ne 0) {
                Save-Latest 'BLOCKED_INVOCATION_RUNTIME' @{ selected_id=$id; fingerprint=$finger; eligible_count=$initialEligible; deferred=$deferred; worker='proven-skill-invocation'; required_skill_key=$requiredSkillKey; forbid_fixed_main=$true; turn_charged=$false; reason='INVOCATION_WORKER_FAILED'; worker_exit_code=$workerCode; worker_output_sha256=(Get-TextSha $workerText); failure=('INVOCATION_WORKER_FAILED exit=' + $workerCode); truth_boundary='Invocation worker ran and failed closed. Original objective is preserved. Do not fall back to fixed:main. Native worker errors are mapped here so the scheduler is not crashed.' } | Out-Null
                return
            }
            Save-Latest 'ROUTED_TO_PROVEN_SKILL_INVOCATION' @{ selected_id=$id; fingerprint=$finger; eligible_count=$initialEligible; deferred=$deferred; worker='proven-skill-invocation'; required_skill_key=$requiredSkillKey; program=$routeProgram; forbid_fixed_main=$true; turn_charged=$false; worker_exit_code=0; truth_boundary='Exact PROVEN skill routed to the invocation worker. A routing/stage receipt is not yet an owner outcome; independent artifact verification is still required.' } | Out-Null
            return
        }
        ...
        [void](Invoke-MissionLease 'Heartbeat' $id ('reports/autonomy-continuation-latest.json')); Save-Latest 'AGENT_TURN_COMPLETED_NOT_OUTCOME_PROOF' @{
            selected_id = $id
            fingerprint = $finger
...
if ($SelfTest) {
    ...
    if (Test-Path -LiteralPath $LeaseBridge -PathType Leaf) {
        $leaseSt = & powershell -NoProfile -File $LeaseBridge -Action SelfTest 2>&1
    ...
    Write-Host 'KEVIN SUPERVISOR v1.8.12 SELFTEST PASS identity_key_budget=true annotation_excluded=true material_boolean_excluded=true selector_first=true capability_router=true skill_lab_not_main=true owner_value_skills_not_main=true proven_skill_before_skill_lab=true invocation_not_main=true invocation_worker_native_error_fail_closed=true public_history_skip_invalid=true deferred_reasons_published=true route_before_turn_charge=true desktop_not_toolless_main=true gateway_agent=fixed-main gateway_rpc_only=true gateway_probe_retries=3 main_preflight=true no_forge_dispatch=true anti_spin=true openclaw_native_node=true shell_shim_bypassed=true native_timeout=180s arbitrary_shell=false authority_expansion=false production_install=false'
    Write-Host 'KEVIN SUPERVISOR v1.8.12 DURABLE-BUDGET PASS per_item_history=true attempts_before_effect=true corruption_fails_closed=true alternating_items_bounded=true'
    Write-Host 'KEVIN SUPERVISOR v1.8.12 WORK-CONSERVATION PASS fingerprint_scoped_cooldown=true alternative_reselection=true local_exclusion_only=true canonical_queue_unchanged=true no_global_idle_with_alternative=true authority_expansion=false'
    exit 0
}
