...
# Kevin Maintenance v1.3.54 StrictMode Idempotent Receipt Wrapper
# Authority-neutral hardening around exact-pinned v1.3.53. Adds only a
# StrictMode-safe install receipt: first-apply results include idempotent=$false
# and status is derived without property-not-found. Does not change Supervisor,
# worker, selector, allowlist, or history. Non-v1812 work is delegated to exact
# parent v1.3.53. Worker native errors remain fail-closed.
...
function Invoke-SelfTest{
    ...
    Write-Host 'KEVIN MAINTENANCE v1.3.3 SELFTEST PASS compatibility=v1.3.54'
    Write-Host 'KEVIN MAINTENANCE v1.3.54 SELFTEST PASS expired_manifest=clean_idle stale_execution=false parent=v1.3.53_exact v1812=allowlisted receipt_idempotent=strict_safe invocation_worker=pinned supervisor_after=v1.8.12 worker_native_error=fail_closed delegation=fail_closed authority_expansion=false arbitrary_shell=false'
}
...
try{
    ...
    Save-State 'ERROR' '' $_.Exception.Message @{authority_effect='NONE';wrapper='1.3.54'}
