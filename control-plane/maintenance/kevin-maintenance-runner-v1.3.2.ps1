    ...
    switch ($Alias) {
        'maintenance_runner' {
            return [pscustomobject]@{
                ...
                Marker = 'KEVIN MAINTENANCE v1.3.2 SELFTEST PASS'
                AllowAbsent = $false
            }
        }
        'work_order_intake' {
        ...
        'os_observer_runner' {
            return [pscustomobject]@{
                Target = (Join-Path $Workspace 'kevin-os-observer.ps1')
                Source = '^control-plane/os-awareness/kevin-os-observer-v[0-9._-]+\.ps1$'
                SelfTestArgs = @('-SelfTest')
                Marker = 'KEVIN OS OBSERVER v0.1 SELFTEST PASS'
                AllowAbsent = $true
            }
        }
        'ui_bridge_runner' {
...
function Install-Pinned([object]$m) {
    ...
    if ($hadTarget) {
        ...
        if (-not [bool]$spec.AllowAbsent -or $cur -ne $AbsentHash) { throw ('expected-current missing alias=' + $alias) }
    }
    ...
    if ($hadTarget) { Copy-Item -LiteralPath $target -Destination $backup -Force }
    try {
        ...
        Invoke-FixedSelfTest $alias $target
        Assert-Benchmark30
        return (New-InstallResult $true $(if($hadTarget){$before}else{$AbsentHash}) (Get-Sha $target) $false $(if($hadTarget){Safe-Text $backup 500}else{''}))
    }
    catch {
...
function Legacy-Stage([string]$Text,[bool]$Apply) {
    ...
    throw 'v1.3.2 never executes legacy executable maintenance packages'
}
function Invoke-SelfTest {
    $base = [pscustomobject]@{
        ...
        source_path = 'control-plane/maintenance/kevin-maintenance-runner-v1.3.2.ps1'
        source_sha256 = ('A' * 64)
    ...
    foreach ($case in @(
        ...
        if ($case[0] -eq 'os_observer_runner') { $x.expected_current_sha256 = $AbsentHash }
        Assert-Replace $x
    }
    $osSpec = Get-AliasSpec 'os_observer_runner'
    if (-not [bool]$osSpec.AllowAbsent -or [string]$osSpec.Target -notmatch 'kevin-os-observer\.ps1$') { throw 'OS observer alias invariant' }
    ...
    Write-Host 'KEVIN MAINTENANCE v1.3.1 SELFTEST PASS compatibility=v1.3.2'
    Write-Host 'KEVIN MAINTENANCE v1.3.2 SELFTEST PASS aliases=4 os_observer_install=approved-read-only absent_install=true ui_restart=1 result_factories=true legacy_autoapply=false arbitrary_shell=false authority_expansion=false max_attempts=3'
}
...
try {
    ...
    if ([int]$m.schema -eq 2 -and [string]$m.kind -eq 'kevin-typed-maintenance-manifest') {
        Save-State 'SCHEMA2_NEEDS_MIGRATION' ([string]$m.id) 'v1.3.2 requires schema-3 aliases for self-maintenance transport.'
        exit 0
    }
