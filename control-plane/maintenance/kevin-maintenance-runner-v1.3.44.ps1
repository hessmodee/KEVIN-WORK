...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.44'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Get-MainModelContractFacts([object]$Config) {
    $r=[ordered]@{main_entry_count=0;model_family='UNKNOWN';model_id_sha256='';provider_api='UNKNOWN';provider_api_sha256='';uses_agent_override=$false;tools_profile='UNKNOWN';root_allowed=@();main_allowed=@();root_denied=@();main_denied=@();main_kevin_core_allowed=$false;read_only=$true}
    $agents=Get-OptionalPropertyValue $Config 'agents';$defaults=Get-OptionalPropertyValue $agents 'defaults'
    $list=Get-OptionalPropertyValue $agents 'list';$rows=@($list|Where-Object{[string](Get-OptionalPropertyValue $_ 'id')-eq'main'})
    $r.main_entry_count=$rows.Count
    if($rows.Count-gt1){throw 'multiple main agent configurations'}
    $main=if($rows.Count-eq1){$rows[0]}else{$null}
    $model=Get-OptionalPropertyValue $main 'model';$r.uses_agent_override=($null-ne$model)
    if($null-eq$model){$model=Get-OptionalPropertyValue $defaults 'model'}
    if($model-isnot[string]){$model=Get-OptionalPropertyValue $model 'primary'}
    $model=[string]$model;$r.model_id_sha256=Get-TextSha256 $model
    if($model-match'(?i)qwen2[.]5'){$r.model_family='QWEN2_5'}elseif($model-match'(?i)qwen3'){$r.model_family='QWEN3'}elseif($model-match'(?i)qwen'){$r.model_family='QWEN_OTHER'}elseif($model){$r.model_family='OTHER'}
    $parts=$model-split'/',2
    if($parts.Count-eq2){
        $providers=Get-OptionalPathValue $Config @('models','providers')
        $provider=Get-OptionalPropertyValue $providers $parts[0]
        $api=[string](Get-OptionalPropertyValue $provider 'api');$r.provider_api_sha256=Get-TextSha256 $api
        if($api-in@('ollama','openai-completions','openai-responses','anthropic-messages')){$r.provider_api=$api}
    }
    $rootTools=Get-OptionalPropertyValue $Config 'tools';$mainTools=Get-OptionalPropertyValue $main 'tools'
    $profile=Get-OptionalPropertyValue $mainTools 'profile';if(-not$profile){$profile=Get-OptionalPropertyValue $rootTools 'profile'}
    if([string]$profile-in@('minimal','coding','messaging','full')){$r.tools_profile=[string]$profile}
    $known=@('read','write','edit','apply_patch','exec','process','web_search','web_fetch','browser','skill_workshop','kevin_system_status','sessions_spawn','sessions_send','group:fs','group:runtime','group:web')
    foreach($scope in @('root','main')){
        $policy=if($scope-eq'root'){$rootTools}else{$mainTools}
        foreach($pair in @(@('allow','allowed'),@('deny','denied'))){
            $values=@(Get-OptionalPropertyValue $policy $pair[0]);$r[$scope+'_'+$pair[1]]=@($known|Where-Object{$values-contains$_})
        }
    }
    $r.main_kevin_core_allowed=($r.main_allowed-contains'kevin_system_status'-or$r.root_allowed-contains'kevin_system_status')
    return $r
}
function Read-MainModelContractFacts {
    $p=Join-Path $env:USERPROFILE '.openclaw\openclaw.json'
    if(-not(Test-Path -LiteralPath $p -PathType Leaf)){throw 'fixed main configuration missing'}
    $cfg=Get-Content -LiteralPath $p -Raw|ConvertFrom-Json
    return Get-MainModelContractFacts $cfg
}
...
function Publish-RuntimeCapabilities {
    ...
    $public=[ordered]@{
        ...
        main_agent_contract=(Read-MainModelContractFacts)
        generated_at=(Get-Date).ToString('o')
        state='OMEN_AUDIT_PROVEN'
        safe_for_public_repo=$true
        openclaw=[ordered]@{
...
function Get-CanaryTranscriptEvidence([object]$Obj,[string]$ExpectedMessage,[datetime]$Started) {
    $e=[ordered]@{present=$false;complete=$false;tool_calls=$null;user_messages=0;assistant_messages=0;final_exact=$false;failure='TRANSCRIPT_UNAVAILABLE';sha256=''}
    ...
    foreach($line in $lines){
        ...
        elseif($role-eq'assistant'){$e.assistant_messages++;$stop=[string](Get-OptionalPropertyValue $m 'stopReason')}
