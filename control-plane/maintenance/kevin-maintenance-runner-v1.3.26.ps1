...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.26'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Install-AutonomyControllerV183([object]$m) {
    ...
    if($supBefore -eq $SupervisorV183Sha -and $selectorBefore -eq $SelectorV11Sha -and $baseSup -eq $SupervisorV183Sha -and $baseForge -eq $ForgeV40Sha){
        Parse-PowerShell $supervisorTarget; & $supervisorTarget -SelfTest; if($LASTEXITCODE-ne0){throw 'installed Supervisor v1.8.3 selftest failed'}
    ...
    if((Get-Sha $supStage)-ne$SupervisorV183Sha){throw 'Supervisor v1.8.3 source hash mismatch'}
    if((Get-Sha $selStage)-ne$SelectorV11Sha){throw 'selector v1.1 source hash mismatch'}
    Parse-PowerShell $supStage
    & $supStage -SelfTest;if($LASTEXITCODE-ne0){throw 'staged Supervisor v1.8.3 selftest failed'}
    Invoke-FixedPythonSelfTest $selStage
    $supText=[IO.File]::ReadAllText($supStage)
    foreach($marker in @('Governed Autonomy Continuation Controller','IDLE_NO_ELIGIBLE_DEMAND','COOLDOWN_BUDGET_EXHAUSTED','AGENT_TURN_COMPLETED_NOT_OUTCOME_PROOF')){if(-not$supText.Contains($marker)){throw ('Supervisor v1.8.3 marker missing: '+$marker)}}
    foreach($bad in @('Invoke-Expression','kevin_shell','Start-Process cmd.exe')){if($supText.Contains($bad)){throw ('Supervisor v1.8.3 forbidden marker: '+$bad)}}
    ...
    try{
        ...
        if((Get-Sha $supervisorTarget)-ne$SupervisorV183Sha){throw 'installed Supervisor v1.8.3 hash mismatch'}
        ...
        Parse-PowerShell $supervisorTarget;& $supervisorTarget -SelfTest;if($LASTEXITCODE-ne0){throw 'installed Supervisor v1.8.3 selftest failed'}
        Invoke-FixedPythonSelfTest $selectorTarget;Assert-Benchmark30
        return [ordered]@{changed=$true;supervisor_before=$supBefore;supervisor_after=(Get-Sha $supervisorTarget);selector_before=$selectorBefore;selector_after=(Get-Sha $selectorTarget);benchmark_supervisor_anchor=$SupervisorV183Sha;forge_anchor=$ForgeV40Sha;selector_first=$true;fixed_main_agent=$true;anti_spin=$true;rollback_available=$true}
    }catch{
...
function Get-AutonomyContinuationJobs {
    $r=Invoke-OpenClawFixedConfig @('automations','list','--all','--json')
...
function Ensure-AutonomyContinuationAutomation {
    ...
    $changed=$false
    if($jobs.Count -eq 0){
        $create=Invoke-OpenClawFixedConfig @('automations','add','--name',$name,'--every','5m','--session','main','--system-event',$event,'--wake','now')
    ...
    $get=Invoke-OpenClawFixedConfig @('automations','get',$id,'--json')
    ...
    $run=Invoke-OpenClawFixedConfig @('automations','run',$id)
