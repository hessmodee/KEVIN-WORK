...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.45'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Ensure-UiBridgeWatchdog {
    ...
    $tmp=$path+'.stage-'+[guid]::NewGuid().ToString('N');[IO.File]::WriteAllText($tmp,$text,$Utf8);Parse-PowerShell $tmp
...
function Refresh-FullAutonomyAssessment {
    ...
    $blockers=@();if(-$benchOk){$blockers+='BENCHMARK_NOT_PASS'};if($drift.Count-gt0){$blockers+='CORE_IDENTITY_DRIFT'};if(-$hbFresh){$blockers+='UI_BRIDGE_STALE'};if($tools-eq0){$blockers+='MAIN_AGENT_TOOL_SURFACE_EMPTY'}
