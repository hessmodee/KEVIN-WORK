param([switch]$SelfTest,[switch]$ProbeOnly)
...
$KnownBackgroundPowerShellTasks = @('Kevin Self-Reliance Watchdog v1','KevinGitHubBridge','KevinNightForge','Kevin UI Bridge v0.3','KevinGatewayKeeper')
...
function Get-WscriptPath {
    foreach ($c in @((Join-Path $env:SystemRoot 'System32\wscript.exe'), (Join-Path $env:SystemRoot 'SysWOW64\wscript.exe'), 'wscript.exe')) {
...
function Test-AlreadyHiddenWrapped([string]$Execute, [string]$Arguments, [string]$VbsPath) {
    $exe = [IO.Path]::GetFileName([string]$Execute)
    if ($exe -notmatch '(?i)^wscript\.exe$') { return $false }
    $a = [string]$Arguments
    if ($a -notmatch [regex]::Escape($VbsPath)) { return $false }
    return $true
}
function Repair-OneConsoleTask([string]$Name, [string]$VbsPath, [string]$Wscript) {
    $task = Get-ScheduledTask -TaskName $Name -ErrorAction SilentlyContinue
    if (-not $task) { return 'ABSENT' }
    $actions = @($task.Actions)
    if ($actions.Count -ne 1) { return 'SKIP_MULTI' }
    $a = $actions[0]
    $execute = [string]$a.Execute
    $args = [string]$a.Arguments
    if (Test-AlreadyHiddenWrapped $execute $args $VbsPath) { return 'OK' }
    $exeName = [IO.Path]::GetFileName($execute)
    if ($exeName -notmatch '(?i)^(powershell|pwsh|cmd)\.exe$') { return 'SKIP_NOT_CONSOLE' }
    ...
    $wrapped = ('"{0}" {1} {2}' -f $VbsPath, $quotedExe, $hiddenArgs).Trim()
...
function Repair-KnownBackgroundConsoleTasks {
    ...
    if (-not $wscript) { return [pscustomobject]@{ Result='NO_WSCRIPT';Changed=0;Failures=1 } }
    $vbs = Get-HiddenRunnerPath
    $names = New-Object System.Collections.Generic.List[string]
    foreach ($n in $KnownBackgroundPowerShellTasks) { if (-not $names.Contains($n)) { $names.Add($n) } }
    try {
        $extra = @(Get-ScheduledTask -ErrorAction SilentlyContinue | Where-Object {
            $_.TaskName -like 'Kevin*' -or $_.TaskName -like 'OpenClaw*'
        })
        foreach ($t in $extra) {
    ...
    foreach ($name in $names) {
        try {
            $r = Repair-OneConsoleTask $name $vbs $wscript
            if ($r -eq 'REPAIRED') { $changed++ }
        } catch { $failures++ }
    }
...
function Invoke-SelfTest {
    ...
    if (-not $PSCommandPath -or -not (Test-Path -LiteralPath $PSCommandPath -PathType Leaf)) { throw 'self path unavailable' }
    ...
    if ($KnownBackgroundPowerShellTasks.Count -lt 4) { throw 'console task allowlist invariant' }
    ...
    if ($selfText -notmatch 'kevin-hidden-run\.vbs') { throw 'hidden vbs runner missing' }
    if ($selfText -notmatch 'Repair-OneConsoleTask') { throw 'vbs wrap repair missing' }
    if ($selfText -notmatch 'SW_HIDE' -and $selfText -notmatch 'Run cmd, 0, False') { throw 'SW_HIDE contract missing' }
    Write-Host 'KEVIN SELF-RELIANCE WATCHDOG v1.7.0 SELFTEST PASS implementation=v1.7.0 console_hygiene=vbs-sw-hide gateway_truth=fresh outage_maintenance_wake=true work_order_poll=true probe_only=true probe_no_intake=true probe_no_restart=true keeper_topology=true safe_startup_classifier=true direct_config_secrets=false retry_max=3 cooldown=15m arbitrary_shell=false caller_argv=false'
}
