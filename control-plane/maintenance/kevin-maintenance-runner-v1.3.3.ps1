...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.3'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Process-Typed([object]$m) {
    Assert-Common $m;if([string]$m.operation -eq 'replace_pinned_component'){Assert-Replace $m}else{Assert-Restart $m};Assert-Governance
    ...
    $attempts++
    try{$result=if([string]$m.operation -eq 'replace_pinned_component'){Install-Pinned $m}else{Restart-UiBridge $m};Save-Attempt ([string]$m.id) $attempts 'PROVEN';Save-State 'APPLIED_PREAUTHORIZED_PROVEN' ([string]$m.id) 'Applied and independently verified.' @{attempts=$attempts;operation=[string]$m.operation;result=$result}}catch{$family=if([string]$m.operation -eq 'replace_pinned_component'){'typed_replace_failure'}else{'ui_bridge_restart_failure'};Save-Attempt ([string]$m.id) $attempts 'FAILED' $family;Save-State 'APPLY_FAILED' ([string]$m.id) $_.Exception.Message @{attempts=$attempts;failure_family=$family};throw}
}

function Legacy-Stage([string]$Text,[bool]$Apply) { $m=$Text|ConvertFrom-Json;if([int]$m.schema -ne 1 -or [string]$m.kind -ne 'kevin-maintenance-manifest'){throw 'legacy manifest invalid'};if(-not$Apply){Save-State 'LEGACY_STAGED_APPROVAL_REQUIRED' ([string]$m.id) 'Legacy executable package requires one-time ApplyOnce.';return};throw 'v1.3.3 never executes legacy executable maintenance packages' }
function Invoke-SelfTest {
    $base=[pscustomobject]@{schema=3;kind='kevin-self-maintenance-manifest';id='selftest-maint-001';authority_class='GREEN';authority_delta='NONE';production_effect='NONE';owner_policy=$OwnerPolicy;preauthorized=$true;operation='replace_pinned_component';target_alias='maintenance_runner';source_path='control-plane/maintenance/kevin-maintenance-runner-v1.3.3.ps1';source_sha256=('A'*64);expected_current_sha256=('B'*64);expected_after_sha256=('A'*64);expires_at=(Get-Date).AddMinutes(10).ToString('o')}
    Assert-Common $base;Assert-Replace $base
    foreach($case in @(@('work_order_intake','control-plane/intake/kevin-work-order-intake-v1.2.3.ps1'),@('skill_lab_runner','control-plane/skill-lab/kevin-skill-lab-v1.0.4.ps1'),@('os_observer_runner','control-plane/os-awareness/kevin-os-observer-v0.1.ps1'),@('self_reliance_watchdog','control-plane/maintenance/kevin-self-reliance-watchdog-v1.3.ps1'))){$x=$base|ConvertTo-Json -Depth 10|ConvertFrom-Json;$x.target_alias=$case[0];$x.source_path=$case[1];if($case[0] -eq 'os_observer_runner'){$x.expected_current_sha256=$AbsentHash};Assert-Replace $x}
    $watch=Get-AliasSpec 'self_reliance_watchdog';if([bool]$watch.AllowAbsent -or [string]$watch.Target -notmatch 'kevin-self-reliance-watchdog\.ps1$'){throw 'watchdog alias invariant'}
    ...
    Write-Host 'KEVIN MAINTENANCE v1.3.2 SELFTEST PASS compatibility=v1.3.3'
    Write-Host 'KEVIN MAINTENANCE v1.3.3 SELFTEST PASS aliases=5 watchdog_transport=fixed os_observer_install=approved-read-only ui_restart=1 arbitrary_shell=false authority_expansion=false max_attempts=3'
}

if($SelfTest){Invoke-SelfTest;exit 0}
try{$text=Get-RemoteManifestText;if($null -eq $text){Save-State 'NO_MANIFEST' '' 'No maintenance proposal is waiting.';exit 0};$m=$text|ConvertFrom-Json;if([int]$m.schema -eq 3 -and [string]$m.kind -eq 'kevin-self-maintenance-manifest'){Process-Typed $m;exit 0};if([int]$m.schema -eq 2 -and [string]$m.kind -eq 'kevin-typed-maintenance-manifest'){Save-State 'SCHEMA2_NEEDS_MIGRATION' ([string]$m.id) 'v1.3.3 requires schema-3 aliases for self-maintenance transport.';exit 0};Legacy-Stage $text ([bool]$ApplyOnce);exit 0}catch{if(-not(Test-Path $LatestPath) -or (Get-Content $LatestPath -Raw) -notmatch 'APPLY_FAILED'){Save-State 'ERROR' '' $_.Exception.Message};Write-Host('MAINTENANCE ERROR '+(Safe-Text $_.Exception.Message 600));exit 1}
