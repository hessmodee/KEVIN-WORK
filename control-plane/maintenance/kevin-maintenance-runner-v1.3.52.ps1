...
# Kevin Maintenance v1.3.52 Supervisor v1.8.11 + Invocation-Worker Compatibility Wrapper
# Authority-neutral hardening around exact-pinned v1.3.51. Adds only
# install_autonomy_controller_v1811. Expired canonical manifests remain a clean
# terminal idle. Non-v1811 work is delegated to exact parent v1.3.51.
...
$ParentCache = Join-Path $ControlPlane 'kevin-maintenance-core-v1.3.51.ps1'
$Repo = 'hessmodee/KEVIN-WORK'
$ParentRepoPath = 'control-plane/maintenance/kevin-maintenance-runner-v1.3.51.ps1'
$ManifestRepoPath = 'inbox/maintenance/manifest.json'
$ExpectedParentSha = '2D7F65C97A2934E7C34FEFF86BD317E84D0E73D6CABD9AC1D99DF0FF32448EAD'
$SupervisorV1811PredecessorSha = 'BDA265ACCB929EC5129B2F38AC76FE276625A604052304FC32C917700BCB532E'
$SupervisorV1811Sha = '685B34F31B797915B6ADC6058FCCDF4AEACD3CDD49D6B084FB31800FA966AB79'
$SupervisorV1811Source = 'control-plane/autonomy/kevin-supervisor-v1.8.11.ps1'
...
$V1811Sources = @($SupervisorV1811Source,$SelectorV12Source,$WorkerV1Source,$BuilderSource,$InvokerSource)
$AllowedOperations = @(
    ...
    'install_autonomy_controller_v1810','install_autonomy_controller_v1811','diagnose_gateway_rpc','run_self_reliance_watchdog_once',
...
function Get-RemoteV1811Bytes([string]$RepoPath){
    if($V1811Sources-notcontains$RepoPath){throw 'v1.8.11 autonomy source path rejected'}
    $endpoint='repos/'+$Repo+'/contents/'+$RepoPath+'?ref=main'
    $b64=Invoke-GhFixed @('api',$endpoint,'--jq','.content')
    if(-not$b64){throw ('v1.8.11 source fetch empty: '+$RepoPath)}
    try{return [Convert]::FromBase64String(($b64-replace'\s',''))}
    catch{throw ('v1.8.11 source base64 invalid: '+$RepoPath)}
}
...
function Invoke-SupervisorV1811SelfTest([string]$Path){
    $old=$ErrorActionPreference
    ...
    if($code-ne0-or$out-notmatch'KEVIN SUPERVISOR v1\.8\.11 SELFTEST PASS'){throw 'Supervisor v1.8.11 fixed selftest failed'}
    if($out-notmatch'route_before_turn_charge=true'-or$out-notmatch'owner_value_skills_not_main=true'){throw 'Supervisor v1.8.11 routing selftest markers missing'}
    if($out-notmatch'proven_skill_before_skill_lab=true'-or$out-notmatch'invocation_not_main=true'){throw 'Supervisor v1.8.11 invocation selftest markers missing'}
}
...
function Assert-AutonomyControllerV1811Install([object]$m){
    $allowed=@('schema','kind','id','authority_class','authority_delta','production_effect','owner_policy','preauthorized','operation','expires_at')
    foreach($p in $m.PSObject.Properties.Name){if($allowed-notcontains[string]$p){throw ('autonomy controller v1.8.11 install manifest must not supply '+[string]$p)}}
...
function Install-AutonomyControllerV1811([object]$m){
    Assert-AutonomyControllerV1811Install $m
    if($env:OS-ne'Windows_NT'){throw 'autonomy controller v1.8.11 install requires Windows'}
    ...
    foreach($p in @($supervisorTarget,$baselineTarget)){if(-not(Test-Path -LiteralPath $p -PathType Leaf)){throw ('v1.8.11 install target missing: '+[IO.Path]::GetFileName($p))}}
    ...
    if($supBefore-eq$SupervisorV1811Sha-and$selectorBefore-eq$SelectorV12Sha-and$workerBefore-eq$WorkerV1Sha-and$builderBefore-eq$BuilderSha-and$invokerBefore-eq$InvokerSha-and$baseSup-eq$SupervisorV1811Sha-and$baseForge-eq$ForgeV40Sha){
        Parse-PowerShell $supervisorTarget
        Parse-PowerShell $workerTarget
        Invoke-SupervisorV1811SelfTest $supervisorTarget
    ...
    if($supBefore-ne$SupervisorV1811PredecessorSha-and$supBefore-ne$SupervisorV1811Sha){throw ('Supervisor v1.8.11 expected-current mismatch actual='+$supBefore)}
    ...
    if($supBefore-eq$SupervisorV1811PredecessorSha-and$baseSup-ne$SupervisorV1811PredecessorSha){throw ('Benchmark baseline Supervisor anchor mismatch actual='+$baseSup)}
    if($supBefore-eq$SupervisorV1811Sha-and$baseSup-ne$SupervisorV1811Sha-and$baseSup-ne$SupervisorV1811PredecessorSha){throw ('Benchmark baseline Supervisor anchor mismatch actual='+$baseSup)}
    if($baseForge-ne$ForgeV40Sha){throw ('Benchmark baseline Forge anchor changed actual='+$baseForge)}

    $supBytes=Get-RemoteV1811Bytes $SupervisorV1811Source
    $selBytes=Get-RemoteV1811Bytes $SelectorV12Source
    $workerBytes=Get-RemoteV1811Bytes $WorkerV1Source
    $builderBytes=Get-RemoteV1811Bytes $BuilderSource
    $invokerBytes=Get-RemoteV1811Bytes $InvokerSource
    $supStage=Join-Path $StageRoot ([string]$m.id+'.supervisor-v1811.ps1')
    ...
    if((Get-Sha $supStage)-ne$SupervisorV1811Sha){throw 'Supervisor v1.8.11 source hash mismatch'}
    ...
    Invoke-SupervisorV1811SelfTest $supStage
    Invoke-SelectorV12SelfTest $selStage

    $supText=[IO.File]::ReadAllText($supStage)
    foreach($marker in @('# Kevin Supervisor v1.8.11 Capability-Aware Continuation Controller','ROUTED_TO_PROVEN_SKILL_INVOCATION','BLOCKED_INVOCATION_RUNTIME','required_skill_key','turn_charged=$false','history_error=$false','kevin-work-selector-v1.2.py',$SelectorV12Sha,'invocation_not_main=true')){
        if(-not$supText.Contains([string]$marker)){throw ('Supervisor v1.8.11 required marker missing: '+[string]$marker)}
    }
    foreach($bad in @('Invoke-Expression','kevin_shell','Start-Process cmd.exe')){if($supText.Contains($bad)){throw ('Supervisor v1.8.11 forbidden marker: '+$bad)}}
    ...
    $baseStageObj.hashes.supervisor=$SupervisorV1811Sha
    ...
    if(([string]$verify.hashes.supervisor).ToUpperInvariant()-ne$SupervisorV1811Sha){throw 'staged Benchmark Supervisor v1.8.11 anchor mismatch'}
    ...
    if(-not$owned){$mutex.Dispose();throw 'Supervisor mutex busy; bounded v1.8.11 install deferred'}
    ...
    try{
        ...
        if($supBefore-eq$SupervisorV1811PredecessorSha-and$liveSup-ne$SupervisorV1811PredecessorSha){throw 'Supervisor changed after v1.8.11 staging; aborting TOCTOU'}
        if($supBefore-eq$SupervisorV1811Sha-and$liveSup-ne$SupervisorV1811Sha){throw 'Supervisor changed after v1.8.11 staging; aborting TOCTOU'}
        ...
        Install-StagedFile $supStage $supervisorTarget $SupervisorV1811Sha
        ...
        if((Get-Sha $supervisorTarget)-ne$SupervisorV1811Sha){throw 'installed Supervisor v1.8.11 hash mismatch'}
        ...
        if(([string]$postBase.hashes.supervisor).ToUpperInvariant()-ne$SupervisorV1811Sha-or([string]$postBase.hashes.forge).ToUpperInvariant()-ne$ForgeV40Sha){throw 'installed Benchmark anchors mismatch'}
        ...
        Invoke-SelectorV12SelfTest $selectorTarget
        Assert-Benchmark30
        return [ordered]@{changed=$true;supervisor_before=$supBefore;supervisor_after=(Get-Sha $supervisorTarget);worker_before=$workerBefore;worker_after=(Get-Sha $workerTarget);selector_before=$selectorBefore;selector_after=(Get-Sha $selectorTarget);benchmark_supervisor_anchor=$SupervisorV1811Sha;forge_anchor=$ForgeV40Sha;history_preserved=$true;work_items_preserved=$true;mission_leases_preserved=$true;route_before_turn_charge=$true;owner_value_skills_to_skill_lab=$true;proven_skill_before_skill_lab=$true;invocation_not_main=$true;invocation_worker_installed=$true;public_history_skip_invalid=$true;rollback_available=$true}
    }catch{
        ...
        throw ('autonomy controller v1.8.11 rollback completed: '+$primary)
    }finally{
...
function Invoke-SelfTest{
    ...
    if($ParentRepoPath-ne'control-plane/maintenance/kevin-maintenance-runner-v1.3.51.ps1'){throw 'parent path widened'}
    if($ManifestRepoPath-ne'inbox/maintenance/manifest.json'){throw 'manifest path widened'}
    if($SupervisorV1811PredecessorSha-ne'BDA265ACCB929EC5129B2F38AC76FE276625A604052304FC32C917700BCB532E'){throw 'v1.8.11 predecessor pin drifted'}
    if($SupervisorV1811Sha-ne'685B34F31B797915B6ADC6058FCCDF4AEACD3CDD49D6B084FB31800FA966AB79'){throw 'v1.8.11 after pin drifted'}
    if($WorkerV1Sha-ne'16C49542847BBB22EACC09F254C030D2FF03DE0ADFB1B9DA08C9B617C73B0332'){throw 'invocation worker pin drifted'}
    if($AllowedOperations-notcontains'install_autonomy_controller_v1811'){throw 'v1811 operation not allowlisted'}
    ...
    $expiredV1811=New-SelfTestManifest (Get-Date).AddMinutes(-5) 'GREEN' 'install_autonomy_controller_v1811'
    $id='';$exp=''
    if(-not(Test-IsCanonicalExpiredObject $expiredV1811 ([ref]$id) ([ref]$exp))){throw 'expired v1811 manifest not classified'}
    $freshV1811=New-SelfTestManifest (Get-Date).AddMinutes(5) 'GREEN' 'install_autonomy_controller_v1811'
    $id='';$exp=''
    if(Test-IsCanonicalExpiredObject $freshV1811 ([ref]$id) ([ref]$exp)){throw 'fresh v1811 classified expired'}
    ...
    $extra=$freshV1811|Select-Object *;$extra|Add-Member -NotePropertyName target_alias -NotePropertyValue 'supervisor' -Force
    try{Assert-AutonomyControllerV1811Install $extra;throw 'extra property accepted'}catch{if($_.Exception.Message-notmatch'must not supply'){throw}}
    Write-Host 'KEVIN MAINTENANCE v1.3.3 SELFTEST PASS compatibility=v1.3.52'
    Write-Host 'KEVIN MAINTENANCE v1.3.52 SELFTEST PASS expired_manifest=clean_idle stale_execution=false parent=v1.3.51_exact v1811=allowlisted invocation_worker=pinned supervisor_after=v1.8.11 delegation=fail_closed authority_expansion=false arbitrary_shell=false'
}
...
try{
    ...
    if($parsed -and [string]$parsed.operation -eq 'install_autonomy_controller_v1811'){
        $result=Install-AutonomyControllerV1811 $parsed
        $status=if([bool]$result.idempotent){'ALREADY_APPLIED_PROVEN'}else{'APPLIED_PREAUTHORIZED_PROVEN'}
        Save-State $status ([string]$parsed.id) 'Supervisor v1.8.11 and invocation worker applied/verified.' @{
            operation='install_autonomy_controller_v1811'
            parent_invoked=$false
            result=$result
            authority_effect='NONE'
        }
        Write-Host ('MAINTENANCE '+$status+' id='+[string]$parsed.id+' operation=install_autonomy_controller_v1811')
        exit 0
    }
    $code=Invoke-Parent
    exit $code
}catch{
    Save-State 'ERROR' '' $_.Exception.Message @{authority_effect='NONE';wrapper='1.3.52'}
