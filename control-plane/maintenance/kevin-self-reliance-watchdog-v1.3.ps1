...
function Invoke-WorkOrderIntakeFixed {
    ...
    return (Invoke-ExactNative 'powershell.exe' @('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',$WorkOrderIntakePath,'-Mode','Poll') 600)
}
...
function WorkOrder-Due([object]$State) {
    ...
    try {
        return ([DateTimeOffset]::Parse([string]$State.last_work_order_at).AddMinutes($WorkOrderIntervalMinutes) -le [DateTimeOffset]::Now)
    }
    catch { return $true }
}
...
function Invoke-SelfTest {
    ...
    Write-Host 'KEVIN SELF-RELIANCE WATCHDOG v1.2 SELFTEST PASS compatibility=v1.3'
    Write-Host 'KEVIN SELF-RELIANCE WATCHDOG v1.3 SELFTEST PASS gateway_status=fixed gateway_restart=fixed maintenance_wake=fixed work_order_poll=fixed cadence=2m runtime_returns=true max_attempts=3 cooldown=15m arbitrary_shell=false caller_argv=false remote_payload=typed-only'
}
