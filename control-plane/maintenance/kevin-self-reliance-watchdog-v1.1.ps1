...
function Read-State{
    ...
    return [pscustomobject]@{
        schema=1;failure_family='';attempts=0;cooldown_until=$null;last_result='';updated_at=$null;
        last_maintenance_at=$null;maintenance_result='NEVER'
    }
}
...
function Invoke-MaintenanceFixed{
    if(-not(Test-Path -LiteralPath $MaintenancePath -PathType Leaf)){return[pscustomobject]@{ExitCode=127;Stdout='';Stderr='maintenance runner missing';TimedOut=$false}}
...
function Save([object]$State,[string]$Result){
    Ensure-Property $State 'last_result' '' ;Ensure-Property $State 'updated_at' $null
    $State.last_result=$Result;$State.updated_at=(Get-Date).ToString('o');Write-JsonAtomic $StatePath $State
}
...
function Run-MaintenanceIfDue([object]$State){
    Ensure-Property $State 'maintenance_result' 'NEVER';Ensure-Property $State 'last_maintenance_at' $null
    if(-not(Maintenance-Due $State)){return$true}
    $m=Invoke-MaintenanceFixed
    $State.last_maintenance_at=(Get-Date).ToString('o')
    if($m.ExitCode-ne0){$State.maintenance_result='FAILED';Save $State 'MAINTENANCE_FAILED';return$false}
    $State.maintenance_result='OK';return$true
}
function Invoke-SelfTest{
    if($MaxAttempts-ne3){throw'watchdog retry invariant'}
    if($CooldownMinutes-ne15){throw'watchdog cooldown invariant'}
    if($MaintenanceIntervalMinutes-ne5){throw'maintenance cadence invariant'}
    $status=@('gateway','status','--require-rpc','--json');$restart=@('gateway','restart','--wait','30s','--json')
    ...
    $q=ConvertTo-Win32CommandLineArg 'C:\Program Files\Kevin\x.ps1';if($q-ne'"C:\Program Files\Kevin\x.ps1"'){throw'Win32 argv quote invariant'}
    if([IO.Path]::GetFileName($MaintenancePath)-ne'kevin-maintenance-runner.ps1'){throw'maintenance target drift'}
    Write-Host 'KEVIN SELF-RELIANCE WATCHDOG v1.1 SELFTEST PASS gateway_status=fixed gateway_restart=fixed maintenance_wake=fixed max_attempts=3 cooldown=15m arbitrary_shell=false caller_argv=false remote_payload=false'
}
if($SelfTest){Invoke-SelfTest;exit 0}

$s=Read-State
foreach($x in @(@('failure_family',''),@('attempts',0),@('cooldown_until',$null),@('last_result',''),@('updated_at',$null),@('last_maintenance_at',$null),@('maintenance_result','NEVER'))){Ensure-Property $s $x[0] $x[1]}
...
if($status.ExitCode-eq0){
    $s.failure_family='';$s.attempts=0;$s.cooldown_until=$null
    if(-not(Run-MaintenanceIfDue $s)){exit 1}
    Save $s 'HEALTHY';exit 0
}
