...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.40'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Run-MainAgentCanary {
    ...
    $statusOk=($status-eq'ok' -or $status-eq'success');$success=($statusOk -and $expected -and $calls-eq0)
    $public=[ordered]@{
