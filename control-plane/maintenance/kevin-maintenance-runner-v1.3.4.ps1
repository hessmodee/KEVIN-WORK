...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.4'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Assert-Common([object]$m) {
    ...
    if(-not(@('replace_pinned_component','restart_ui_bridge','audit_runtime_convergence','replace_runtime_policy_bundle') -contains [string]$m.operation)){throw 'operation not allowlisted'}
}
