...
$SupervisorV183Sha = '8A5B81A7A2BFC81DDF776055C6C7250E6B63D49FA27AF45A523245B7DF19B636'
$SelectorV11Sha = '9DF1F770E8855232758AC275FA2D3A82C6D484099B5B2D2185A999ACDE185DE4'
$SupervisorV183Source = 'control-plane/autonomy/kevin-supervisor-v1.8.7.ps1'
# Legacy v183 symbol/operation name is retained as the typed contract identifier; its qualified target is Supervisor v1.8.7.
...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.42'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Install-AutonomyControllerV183([object]$m) {
    ...
    if($supBefore -eq $SupervisorV183Sha -and $selectorBefore -eq $SelectorV11Sha -and $baseSup -eq $SupervisorV183Sha -and $baseForge -eq $ForgeV40Sha){
        Parse-PowerShell $supervisorTarget; & $supervisorTarget -SelfTest; if($LASTEXITCODE-ne0){throw 'installed Supervisor v1.8.7 selftest failed'}
    ...
    if((Get-Sha $supStage)-ne$SupervisorV183Sha){throw 'Supervisor v1.8.7 source hash mismatch'}
    if((Get-Sha $selStage)-ne$SelectorV11Sha){throw 'selector v1.1 source hash mismatch'}
    Parse-PowerShell $supStage
    & $supStage -SelfTest;if($LASTEXITCODE-ne0){throw 'staged Supervisor v1.8.7 selftest failed'}
    Invoke-FixedPythonSelfTest $selStage
    $supText=[IO.File]::ReadAllText($supStage)
    foreach($marker in @('Governed Autonomy Continuation Controller','IDLE_NO_ELIGIBLE_DEMAND','BOUNDED_TURNS_REQUIRES_NEW_EVIDENCE','AGENT_TURN_COMPLETED_NOT_OUTCOME_PROOF')){if(-not$supText.Contains($marker)){throw ('Supervisor v1.8.7 marker missing: '+$marker)}}
    foreach($bad in @('Invoke-Expression','kevin_shell','Start-Process cmd.exe')){if($supText.Contains($bad)){throw ('Supervisor v1.8.7 forbidden marker: '+$bad)}}
    ...
    try{
        ...
        if((Get-Sha $supervisorTarget)-ne$SupervisorV183Sha){throw 'installed Supervisor v1.8.7 hash mismatch'}
