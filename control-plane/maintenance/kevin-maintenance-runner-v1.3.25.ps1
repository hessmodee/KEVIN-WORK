...
function Save-State([string]$Status,[string]$ManifestId = '',[string]$Detail = '',[hashtable]$Extra = $null) {
    $obj = [ordered]@{ schema=3; kind='kevin-maintenance-state'; version='1.3.25'; at=(Get-Date).ToString('o'); status=$Status; manifest_id=$ManifestId; detail=(Safe-Text $Detail) }
...
function Process-Typed([object]$m) {
    ...
    switch([string]$m.operation){
        ...
        'install_autonomy_controller_v182' { Assert-AutonomyControllerV182Install $m }
    ...
    try{
        $result=switch([string]$m.operation){
            ...
            'install_autonomy_controller_v182' { Install-AutonomyControllerV182 $m; break }
        }
        ...
        $family=switch([string]$m.operation){
            ...
            'install_autonomy_controller_v182' {'autonomy_controller_v182_install_failure'}
...
function Invoke-SelfTest {
    ...
    $aci=$audit|ConvertTo-Json -Depth 10|ConvertFrom-Json;$aci.operation='install_autonomy_controller_v182';Assert-Common $aci;Assert-AutonomyControllerV182Install $aci
