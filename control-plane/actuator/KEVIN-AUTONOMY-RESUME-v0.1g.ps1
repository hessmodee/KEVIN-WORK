...
$OldActuatorBlob='ef1e7f9ea6847e6d963af0cd40c28735573bc26d'
$NewActuatorBlob='489dfd2277245341a790c00a5db04ecd3ffb4fab'
...
$oldManifest=Join-Path $Root 'install-manifest-v0.1g.json'
if(Test-Path -LiteralPath $oldManifest){
 try{
  $m=Get-Content -LiteralPath $oldManifest -Raw|ConvertFrom-Json
  foreach($jid in @($m.reconciler_job_id,$m.telemetry_job_id)|Where-Object{$_}){try{$null=Invoke-OpenClawJson -Args @('automations','remove',[string]$jid,'--json')}catch{}}
 }catch{}
}
...
$rId=$null;$bId=$null
try{
 $rc=Invoke-OpenClawJson -Args @('automations','create','*/3 * * * *','--name','Kevin Autonomy Reconciler v0.1','--command-argv',$reconcileArgv,'--command-cwd',$Root,'--timeout-seconds','600','--no-output-timeout-seconds','600','--output-max-bytes','65536','--no-deliver','--json')
 $rId=Resolve-JobId -Object $rc -Label 'reconciler'
 $rj=Invoke-OpenClawJson -Args @('automations','get',$rId,'--json')
 ...
 $bc=Invoke-OpenClawJson -Args @('automations','create','*/5 * * * *','--name','Kevin Autonomy Telemetry v0.1','--command-argv',$bridgeArgv,'--command-cwd',$Root,'--timeout-seconds','120','--no-output-timeout-seconds','120','--output-max-bytes','32768','--no-deliver','--json')
 $bId=Resolve-JobId -Object $bc -Label 'telemetry'
 $bj=Invoke-OpenClawJson -Args @('automations','get',$bId,'--json')
 ...
 if($rId){try{$null=Invoke-OpenClawJson -Args @('automations','remove',$rId,'--json')}catch{}}
 if($bId){try{$null=Invoke-OpenClawJson -Args @('automations','remove',$bId,'--json')}catch{}}
 throw
}
...
Write-JsonAtomic -Object $install -Path (Join-Path $EvidenceDir ("$Stamp-install-proof-v0.1g.json"))
$rollback=@"
`$ErrorActionPreference='Continue'
openclaw automations remove '$rId' --json | Out-Null
openclaw automations remove '$bId' --json | Out-Null
Write-Host 'Kevin Autonomy v0.1g jobs removed. Backup: $Backup'
"@
[IO.File]::WriteAllText((Join-Path $Root 'ROLLBACK-v0.1g.ps1'),$rollback,(New-Object Text.UTF8Encoding($false)))

Write-Host "`n============================================================" -ForegroundColor Green
Write-Host 'KEVIN AUTONOMY ACTUATOR v0.1g INSTALLED + PROVEN' -ForegroundColor Green
...
Write-Host "Rollback: $(Join-Path $Root 'ROLLBACK-v0.1g.ps1')"
