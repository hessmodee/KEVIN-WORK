...
$OldActuatorBlob='38bc72bc29569c72c3618b0b8ce8fa59876ba76c'
$NewActuatorBlob='ef1e7f9ea6847e6d963af0cd40c28735573bc26d'
...
$oldManifest=Join-Path $Root 'install-manifest-v0.1f.json'
...
Write-JsonAtomic -Object $install -Path (Join-Path $EvidenceDir ("$Stamp-install-proof-v0.1f.json"))
$rollback=@"
`$ErrorActionPreference='Continue'
openclaw automations remove '$rId' --json | Out-Null
openclaw automations remove '$bId' --json | Out-Null
Write-Host 'Kevin Autonomy v0.1f jobs removed. Backup: $Backup'
"@
[IO.File]::WriteAllText((Join-Path $Root 'ROLLBACK-v0.1f.ps1'),$rollback,(New-Object Text.UTF8Encoding($false)))

Write-Host "`n============================================================" -ForegroundColor Green
Write-Host 'KEVIN AUTONOMY ACTUATOR v0.1f INSTALLED + PROVEN' -ForegroundColor Green
...
Write-Host "Rollback: $(Join-Path $Root 'ROLLBACK-v0.1f.ps1')"
