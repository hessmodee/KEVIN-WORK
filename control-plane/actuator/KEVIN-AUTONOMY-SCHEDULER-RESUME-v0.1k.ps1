...
$manifest=[ordered]@{schema=1;version='0.1k';installed_at=(Get-Date).ToString('o');openclaw_version=$script:Ocl.Version;native_transport='ProcessStartInfo-Win32-argv-quoting-v1';actuator_blob=$ActuatorBlob;bridge_blob=$NewBridgeBlob;reconciler_job_id=$rId;telemetry_job_id=$bId;removed_prior_job_ids=@($removed);remote_autonomy_sha=[string]$remoteObj.sha}
$manifestPath=Join-Path $Root 'install-manifest-v0.1k.json';Write-JsonAtomic $manifest $manifestPath;Write-JsonAtomic $manifest (Join-Path $EvidenceDir ("$Stamp-install-proof-v0.1k.json"))
...
Write-Host 'Kevin Autonomy scheduler v0.1k jobs removed.'
"@
$rollbackPath=Join-Path $Root 'ROLLBACK-v0.1k.ps1';[IO.File]::WriteAllText($rollbackPath,$rollback,(New-Object Text.UTF8Encoding($false)))

Write-Host "`n============================================================" -ForegroundColor Green
Write-Host 'KEVIN AUTONOMY SCHEDULER v0.1k INSTALLED + PROVEN' -ForegroundColor Green
