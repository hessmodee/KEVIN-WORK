...
$Receipt = Join-Path $EvidenceRoot 'bootstrap-self-reliance-v1.2-proven.json'
...
function Invoke-BootstrapSelfTest {
    ...
    Write-Host 'KEVIN SELF-RELIANCE BOOTSTRAP v1.2 SELFTEST PASS components=3 safe_runtime=true immutable_ref=true exact_hashes=true task_state_check=true task=limited rollback=true arbitrary_shell=false caller_payload=false'
}
...
function Invoke-Preflight {
    ...
    try {
        ...
        Write-Host 'KEVIN SELF-RELIANCE BOOTSTRAP v1.2 PREFLIGHT PASS api_fetch=true component_selftests=true'
    }
...
function Invoke-TaskContractTest {
    ...
    try {
        ...
        Write-Host ('TASK_CONTRACT_PASS state=' + [string]$t.State + ' runlevel=' + [string]$t.Principal.RunLevel)
    }
    finally {
...
try {
    Write-Host '=== KEVIN SELF-RELIANCE BOOTSTRAP v1.2 ==='
