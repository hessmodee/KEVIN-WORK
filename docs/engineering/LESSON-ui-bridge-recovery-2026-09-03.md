...
Observed recovery outcome: manifest bess-ui-recovery-20260903011136 returned APPLY_FAILED at 2026-09-02T19:15:32-06:00: UI Bridge did not produce fresh READY heartbeat. The request was archived and retired after one attempt. A later collection saw a newer but non-continuing heartbeat; that is not recovery proof. Do not queue the Notepad exercise while the UI connection remains unhealthy. Bess prepared the read-only owner evidence collector (PR43), with Windows PowerShell 5.1 CI proof, because installed source and task startup details cannot be exported through the current bridge.
