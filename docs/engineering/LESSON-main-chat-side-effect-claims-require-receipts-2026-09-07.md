...
A successful `kevin_app_launch` receipt can support only the launch claim covered by that tool. It cannot support claims such as:

- text was typed,
- an image was drawn,
- a file was saved,
- a dialog was accepted,
- a requested artifact exists.

Each stronger claim needs its own governed capability and postcondition.
...
1. Put a compact **Execution integrity — highest priority** block at the top of `workspace/AGENTS.md` and `workspace/SOUL.md`.
...
## Why a finalize-hook output filter is not the immediate fix

OpenClaw documentation confirms workspace files such as `AGENTS.md`, `SOUL.md`, and `MEMORY.md` are injected into normal agent context, while the `AGENTS.md` Tools section is guidance rather than the mechanism that grants tool availability.

The installed Kevin screenshot shows OpenClaw `2026.7.1-2`. A documented upstream issue for the 2026.7.1 embedded-agent path reports that `llm_output`, `before_agent_finalize`, and `agent_end` did not reliably fire for non-Codex providers, including Ollama-class embedded providers. Therefore Kevin must **not** depend on `before_agent_finalize` as the sole deterministic truth guard on this installed version.

Use bootstrap doctrine + deterministic evals + typed capability contracts now. A future OpenClaw upgrade/output-interception guard may be considered only after isolated Windows/Ollama validation and full rollback proof.
...
- fixed roots only (e.g. Kevin workspace and explicitly approved Desktop output area),
