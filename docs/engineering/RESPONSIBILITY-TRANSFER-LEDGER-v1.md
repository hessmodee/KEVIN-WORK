...
Read-only history recovery is now CI-PROVEN, not an installed repair: run33613649441 at e51132024dfd8b762506f4b0c66a03eafcd8d53f passed18 tests on Linux and Windows with all489 tracked files unchanged. Exact published receipts recover staging's earlier3-turn fingerprint plus a later1-turn fingerprint (observed lower bound4), and HQ's3 turns, without counting repeated snapshots again. The proposal remains history_complete=false, family mapping unresolved, remaining_budget=null and reopen_authorized=false. See docs/engineering/KEVIN-HISTORY-RECOVERY-CANDIDATE-2026-09-02.md and docs/engineering/history-recovery-proposal-20260902.json. Candidate code stays isolated on chief-engineer/history-recovery-20260902. This does not repair the installed v1.8.8 controller or satisfy its strict14-case runtime gate. Next: validate migration completeness/family provenance, implement append-only history and fail-closed bootstrap/recovery, add real crash/effect-replay tests, then qualify the existing typed path. Do not reset budgets, reopen completed work, release the isolation hold or grant T-level credit.

The earlier strict baseline gate remains authoritative for its tested scope: run33608179462 reproduced6 contract violations across14 cases; no runtime pass is claimed. Additional family/reopening/real-crash/effect tests remain required.
