...
function workerState(key,d,s){
 ...
 if(key==='tick'){
   ...
   if(t && /growth|now-doing|same_cycle|operate_new|uia|paint|notepad|minecraft/i.test(String(t.source||'')+' '+tt)) return 'working';
   return 'ready';
 }
 if(key==='ollama'){
...
async function load(){
 ...
 try{nowDoing=await fetch(RAW+'now-doing.json?'+Date.now()).then(r=>r.ok?r.json():null)}catch(e){nowDoing=null}
 if(nowDoing && nowDoing.doing){
  if(!d.current_task) d.current_task={};
  d.current_task.title=nowDoing.doing;
  d.current_task.phase=nowDoing.doing;
  d.current_task.source='now-doing';
  d.current_task.started_at=nowDoing.generated_at||d.current_task.started_at;
  if(!d.services) d.services={};
  if(nowDoing.chat_18789) d.services.gateway='healthy';
  if(nowDoing.ollama_11434) d.services.ollama='healthy';
  d.services.tick='healthy';
 }
