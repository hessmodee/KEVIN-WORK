...
function renderKevinCenter(d,s,state){
 const sup=s?.supervisor||{},badge=document.getElementById('kevinState');
 badge.textContent=stateLabel(state);badge.style.setProperty('--kstatec',stateColor(state));badge.className=`loop state-${state}`;
...
async function load(){
 let d={},s={};
 try{[d,s]=await Promise.all([fetch(RAW+'dashboard-state.json?'+Date.now()).then(r=>r.json()),fetch(RAW+'support-latest.json?'+Date.now()).then(r=>r.json())])}catch(e){document.getElementById('newsText').textContent='Telemetry fetch failed: '+e.message;document.getElementById('kevinState').textContent='OFFLINE';document.getElementById('kevinState').style.setProperty('--kstatec',stateColor('offline'));return}
 ...
 document.getElementById('activeWorkers').textContent=active.length?`${active.length} active`:'0 active · available lanes ready';document.getElementById('subagents').innerHTML=active.map(w=>owl(w.c).replace('class="owl"','class="subowl"')).join('');
