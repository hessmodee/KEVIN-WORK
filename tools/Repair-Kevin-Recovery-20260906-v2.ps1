...
$RepairRoot = Join-Path $Workspace 'reports\maintenance\platform-recovery-20260906'
$ReceiptPath = Join-Path $Workspace 'reports\maintenance\platform-recovery-20260906-latest.json'
...
function Assert-CurrentConfigContract([object]$Config) {
    ...
    if ($null -eq $modelValue) { $modelValue = Get-Prop $defaults 'model' }
    $primary = if ($modelValue -is [string]) { [string]$modelValue } else { [string](Get-Prop $modelValue 'primary') }
    if ($primary -cne 'ollama-chat-16k/qwen2.5:14b') { throw 'Main repaired model contract changed' }

    $provider = Get-Prop (Get-Prop (Get-Prop $Config 'models') 'providers') 'ollama-chat-16k'
    ...
    $models = @(@(Get-Prop $provider 'models') | Where-Object { [string](Get-Prop $_ 'id') -ceq 'qwen2.5:14b' })
    if ($models.Count -ne 1) { throw 'Expected one repaired qwen2.5:14b model entry' }
    if ([int64](Get-Prop $models[0] 'contextTokens') -ne 16384 -or
        [int64](Get-Prop (Get-Prop $models[0] 'params') 'num_ctx') -ne 16384) {
        throw '16k model limits changed'
    }
...
function Get-Preflight {
    ...
    if ((Get-CanonicalJson $config) -cne (Get-CanonicalJson $trusted)) {
        throw 'SAFE STOP: current config differs semantically from proven exact-five config beyond touch metadata'
    }
...
function Invoke-SelfTest {
    ...
    Write-Host 'KEVIN RECOVERY V2 SELFTEST PASS semantic_equivalence=true r04_one_leaf=true rollback=true ui_rebuild=true notepad_roundtrip=true arbitrary_shell=false'
}
...
$mutex = New-Object Threading.Mutex($false,'Global\KevinPlatformRecovery20260906V2')
$owned = $false
try {
    ...
    if ($CheckOnly) {
        Write-Host ('KEVIN_RECOVERY_V2_READY trusted_config=found semantic_equivalent=true r04=qualified ui_task=' + $preflight.ui.ok + ' ui_reason=' + $preflight.ui.reason)
        exit 0
    }
    New-Item -ItemType Directory -Path $RepairRoot -Force | Out-Null
    $receipt = [ordered]@{schema=2;kind='kevin-platform-recovery-20260906';started_at=(Get-Date).ToString('o');status='STARTED';r04=$null;ui=$null;ui_proof=$null;final_benchmark=$null}
    ...
    try {
        ...
        Write-JsonAtomic $ReceiptPath $receipt
        Write-Host 'KEVIN_RECOVERY_V2_APPLIED_PROVEN benchmark=30/30 ui=sustained ui_notepad=ROUND_TRIP_PROVEN'
    } catch {
