...
if (-not (Test-Path $freeze)) { throw "STOP: missing frozen plugin $freeze" }
if ($cfg -notmatch "openclaw-reader") { throw "STOP: refusing to write non-reader config" }

Write-Host "===== 1) dirs ====="
New-Item -ItemType Directory -Force -Path $ws, (Join-Path $state "extensions") | Out-Null
...
Write-Host "===== 3) copy frozen plugin ====="
...
Require-Ok "writer reader config"
...
if (-not (Test-Path $openclawJs)) { throw "STOP: missing $openclawJs" }
$cmd = @"
@echo off
...
try {
  Unregister-ScheduledTask -TaskName "KevinReaderGateway" -Confirm:$false -ErrorAction SilentlyContinue
} catch {}
...
Write-Host "===== 11) PONG inspect (no status question yet) ====="
Set-ReaderEnv
openclaw --profile reader agent --agent kevin-reader --message "/new" | Out-Null
...
$names = @($entries | ForEach-Object { if ($_.name) { $_.name } else { "$_" } })
$calls = 0
...
Write-Host "production config was not modified."
