...
once("    try{$w=Get-Content -LiteralPath $statePath -Raw|ConvertFrom-Json}catch{throw 'watchdog state invalid JSON'}\n    $facts=Get-DirectGatewayConfigFacts;$top=Get-GatewayTopology",
...
once("state='OMEN_OPERATIONAL_PROOF';safe_for_public_repo=$true;watchdog_sha256=Get-Sha $p;last_result=[string]$w.last_result",
     ...
     'public-proof-state')
once("source_contract='One fixed watchdog run plus fixed metadata-only config/topology inspection. No caller-selected command, argv, path, token, or raw output.'",
     ...
     'source-contract')
once("return [ordered]@{state='OMEN_OPERATIONAL_PROOF';receipt_sha256=$receipt;last_result=$public.last_result",
     ...
     'return-state')
once("    Write-Host 'KEVIN MAINTENANCE v1.3.32 SELFTEST PASS gateway_listener_pid_collision=false native_npm_cli=true partial_install_rollback=true fixed_listener_identity_check=true rollback=true arbitrary_shell=false authority_expansion=false'",
     "    Write-Host 'KEVIN MAINTENANCE v1.3.32 SELFTEST PASS gateway_listener_pid_collision=false native_npm_cli=true partial_install_rollback=true fixed_listener_identity_check=true rollback=true arbitrary_shell=false authority_expansion=false'\n    $wp=(Get-Command Run-SelfRelianceWatchdogOnce).ScriptBlock.ToString();if($wp -notmatch "'-ProbeOnly'"){throw 'watchdog ProbeOnly invocation missing'};if($wp -notmatch 'implementation=v1\\\\.6\\\\.0'){throw 'watchdog v1.6.0 contract gate missing'};if($wp -match "'-File',\\$p\\) 240"){throw 'recursive full-watchdog invocation retained'}\n    Write-Host 'KEVIN MAINTENANCE v1.3.33 SELFTEST PASS watchdog_probe=v1.6.0 probe_no_intake=true probe_no_restart=true classifier_proof=true native_npm_cli=true package_rollback=true arbitrary_shell=false authority_expansion=false'",
     'selftest')
