...
AI-HANDOVER.md is the one human current handover. The JSON twin is machine
state only. The builder fingerprints turnover-relevant semantics rather than
raw telemetry bytes so frequent heartbeat/report timestamps do not continually
move main underneath active engineering work. Incoming agents are still told
to read the fresh runtime reports directly.
"""
...
def semantic_checkpoint(
    ...
    maintenance = dot(support, "maintenance", default={})
    return {
        ...
        "maintenance": {
            "status": maintenance.get("status") if isinstance(maintenance, dict) else None,
            "detail": maintenance.get("detail") if isinstance(maintenance, dict) else None,
        },
        "queues": {
        ...
        "cron": normalize_cron(dot(support, "cron", "jobs", default=[])),
    }
...
def main() -> None:
    ...
    maintenance_status = dot(support, "maintenance", "status", default="UNKNOWN")
    maintenance_detail = dot(support, "maintenance", "detail", default="")

    semantic = semantic_checkpoint(task, engineering, support, autonomy)
    semantic_fingerprint = sha256_text(json.dumps(semantic, sort_keys=True, separators=(",", ":")))

    previous_twin = read_json(HANDOVER_JSON)
    if (
        previous_twin.get("schema") == 4
        and previous_twin.get("semantic_fingerprint_sha256") == semantic_fingerprint
    ...
    twin = {
        ...
        "runtime_snapshot": {
            "engineering_at_checkpoint": eng_at,
            "support_at_checkpoint": sup_at,
            "autonomy_at_checkpoint": auto_at,
            "benchmark": {
                "status": benchmark_status,
                "passed": benchmark_passed,
                "total": benchmark_total,
...
- Maintenance: **{maintenance_status}** — {maintenance_detail}
