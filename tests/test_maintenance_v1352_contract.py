#!/usr/bin/env python3
"""Static contract for Maintenance v1.3.52 / Supervisor v1.8.11 install."""
...
RUNNER = ROOT / "control-plane" / "maintenance" / "kevin-maintenance-runner-v1.3.52.ps1"
PARENT = ROOT / "control-plane" / "maintenance" / "kevin-maintenance-runner-v1.3.51.ps1"
SUPERVISOR = ROOT / "control-plane" / "autonomy" / "kevin-supervisor-v1.8.11.ps1"
...
PARENT_SHA = "2D7F65C97A2934E7C34FEFF86BD317E84D0E73D6CABD9AC1D99DF0FF32448EAD"
SUPERVISOR_BEFORE = "BDA265ACCB929EC5129B2F38AC76FE276625A604052304FC32C917700BCB532E"
SUPERVISOR_AFTER = "685B34F31B797915B6ADC6058FCCDF4AEACD3CDD49D6B084FB31800FA966AB79"
...
class MaintenanceV1352ContractTests(unittest.TestCase):
    @classmethod
    ...
    def test_parent_and_component_pins_match_tree(self) -> None:
        ...
        for pin in (PARENT_SHA, SUPERVISOR_BEFORE, SUPERVISOR_AFTER, WORKER_SHA, BUILDER_SHA, INVOKER_SHA, SELECTOR_SHA):
            self.assertIn(pin, self.text)

    def test_selftest_marker_and_v1811_allowlist(self) -> None:
        self.assertIn("KEVIN MAINTENANCE v1.3.3 SELFTEST PASS", self.text)
        self.assertIn("compatibility=v1.3.52", self.text)
        self.assertIn("install_autonomy_controller_v1811", self.text)
        self.assertIn("Install-AutonomyControllerV1811", self.text)
        ...
        self.assertIn("control-plane/autonomy/kevin-supervisor-v1.8.11.ps1", self.text)
    ...
    def test_fail_closed_fetch_allowlist(self) -> None:
        self.assertIn("v1.8.11 autonomy source path rejected", self.text)
