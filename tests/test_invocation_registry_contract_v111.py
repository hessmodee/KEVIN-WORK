...
class InvocationRegistryContractV111Tests(unittest.TestCase):
    ...
    def test_repair_script_fetches_versioned_paths(self) -> None:
        ...
        self.assertIn("PullerVersion = 'v1.6'", puller)
        self.assertIn(INV112_SHA, puller)
        self.assertIn(BLD103_SHA, puller)
        self.assertIn("Diagnose-Kevin-InvocationStage-v1.ps1", puller)
        self.assertIn("Repair-Kevin-StickyInvokeState-v1.ps1", puller)
        self.assertIn("worker_sha256", puller)
        ...
        self.assertIn("INVOCATION_ID_REUSED_WITH_DIFFERENT_REQUEST", diagnose)
        self.assertIn("diagnose-worker-sim", diagnose)
        self.assertIn("invoke-owner-west-motor-parts-chase-fresh-8-v1", diagnose)
        self.assertIn("7E1129B7FE2B21C90EED634B7A1A356B55630B56DD700A15A7A8C307AEB827AE", diagnose)
        self.assertIn("SkipDiagnose", text)
        sticky = (ROOT / "tools" / "Repair-Kevin-StickyInvokeState-v1.ps1").read_text(encoding="utf-8")
        self.assertIn("invoke-invoke-owner-west-motor-parts-chase-fresh-8-v1", sticky)
        self.assertIn("Does not recopy Supervisor", sticky)
        self.assertIn("ALREADY_CLEAN", sticky)
